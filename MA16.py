...
# Loop to collect user input for each currency
for currency in currencies:
    ...
    for i, (indicator, weight) in enumerate(indicators.items()):
        with cols[i]:
            # Check if the input value exists in session_state, otherwise default to 0.0
            key = f"{currency}_{indicator}"
            if key not in st.session_state:
                st.session_state[key] = 0.0  # Initialize session state for each currency-indicator pair

            score = st.number_input(
                f"{indicator}",
                value=st.session_state[key],  # Default value from session state
            ...
            # Update the session state with the user's input
            st.session_state[key] = score  # Store the score back in session state
